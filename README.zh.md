...
- ⚡ **一个快捷键**（`F1`，可改）→ 原生框选 → 预览 → 进剪贴板，随处粘贴。
- 🎛️ **控制权在你**：预览里 `C` 复制压缩图，`⇧C` 复制原图（不压缩）。
...
- 首次启动会有个欢迎窗：可勾选**开机自启**、并看到当前快捷键。
- 菜单栏出现 📸 图标（无 Dock 图标，后台 `LSUIElement`）。
- 按 **`F1`**（默认，可在 📸 → Shortcut 改）→ 拖拽框选一块区域。
- 弹出预览：**`C`** 或 **`↩`** 复制压缩图 · **`⇧C`** 复制原图（不压缩）· **`Esc`** 取消，然后 `⌘V` 粘给 agent。

> 很多 Mac 的 `F1` 默认是「调暗屏幕」，可能要按 `fn`+`F1`，或在菜单里把快捷键换成 `⌘⇧2`。
...
无论哪档，每张图都自动硬压到 **< 1000KB**。菜单里还有 **Shortcut 快捷键**（F1 / F2 / ⌘⇧2 / ⌘⇧5）和 **开机自启**，都自动记住、无需配置文件。
